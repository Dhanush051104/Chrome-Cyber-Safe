(function () {
  ...
  function testBackendConnection() {
    chrome.runtime.sendMessage({ action: "TEST_BACKEND" }, (response) => {
      if (response?.success) {
  ...
  function getThreatBodyText() {
    if (isPhishingDemo()) return "Chrome Cyber Safe detected a suspicious banking verification flow and blocked the action before navigation.";
    if (isGmailDemo()) return "Chrome Cyber Safe detected a suspicious email-based phishing flow and blocked the action before navigation.";
    if (isSocialDemo()) return "Chrome Cyber Safe detected a social-media impersonation and manipulated-media trap, and blocked a high-risk action before exploitation.";
    return "Chrome Cyber Safe blocked a suspicious interaction before navigation.";
  }
  ...
  function extractFeatureVector() {
    ...
    const urgencyKeywords = ["urgent", "immediately", "verify now", "action required", "within 20 minutes", "within 30 minutes", "avoid suspension", "suspended"];
    const bankingKeywords = ["bank", "banking", "otp", "password", "login", "security alert", "verify"];
    const emailScamKeywords = ["internship", "job", "offer", "eligibility", "mail", "gmail", "selection"];
    const socialKeywords = ["recover account", "fake account", "impersonation", "photo misuse", "morphed", "manipulated", "reported", "review report", "support center"];
    const downloadKeywords = ["download", "evidence", ".zip", "attachment", "file", "proof"];
    ...
    const featureVector = [
      countMatches(text, urgencyKeywords),
      countMatches(text, bankingKeywords),
      countMatches(text, emailScamKeywords),
      countMatches(text, socialKeywords),
      countMatches(text, downloadKeywords),
      riskyElementsCount,
      morphCount,
      isPhishingDemo() ? 1 : 0,
      isGmailDemo() ? 1 : 0,
  ...
  function calculateTensorRiskScore() {
    try {
      ...
      score = Math.round((1 / (1 + Math.exp(-score))) * 100);
      ...
      chrome.storage.local.set({ aiRiskScore: score, aiEngine: "Lightweight Local Model" });
      return score;
  ...
  function saveBlockedSite(siteUrl, score, sourceType) {
    chrome.storage.local.get(["blockedSites", "threatsBlocked"], (data) => {
      ...
      blockedSites.unshift({ url: siteUrl, score: score, time: new Date().toLocaleTimeString(), source: sourceType });
      chrome.storage.local.set({ blockedSites: blockedSites.slice(0, 12), threatsBlocked: threatsBlocked + 1, lastRiskScore: score, protectionStatus: "ACTIVE" });
    });
  }
  ...
  function createTopLoader() {
    ...
    loader.id = "ss-loader";
    loader.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:999999;background:linear-gradient(90deg,#0f172a,#1e293b);color:white;padding:10px 18px;font-family:Arial,sans-serif;font-size:13px;font-weight:700;display:flex;align-items:center;gap:10px;box-shadow:0 2px 12px rgba(0,0,0,0.25);animation:ssFadeIn 0.3s ease;";
    loader.innerHTML = `<div style="width:16px;height:16px;border:2px solid rgba(255,255,255,0.35);border-top-color:#fff;border-radius:50%;animation:ssSpin 0.8s linear infinite;"></div><span>🧠 Chrome Cyber Safe is analyzing page risk...</span>`;
  ...
  function createTopBanner(riskScore) {
    ...
    banner.id = "ss-banner";
    banner.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:999998;background:linear-gradient(90deg,#7f1d1d,#991b1b);color:white;padding:12px 18px;font-family:Arial,sans-serif;font-size:14px;font-weight:700;box-shadow:0 2px 12px rgba(0,0,0,0.25);display:flex;justify-content:space-between;align-items:center;gap:10px;animation:ssFadeIn 0.35s ease;";
    banner.innerHTML = `<span>🛡️ Chrome Cyber Safe Active • Real-time threat scoring enabled</span><span id="ss-risk-score" style="background:rgba(255,255,255,0.14);padding:6px 10px;border-radius:999px;font-size:12px;font-weight:800;">Risk Score: 0</span>`;
  ...
  function showQuarantineScreen() {
    ...
    const subtitle = isSocialDemo() ? "Chrome Cyber Safe has isolated this fake social-media threat" : "Chrome Cyber Safe has isolated this phishing page";
      ...
      <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:radial-gradient(circle at top right,rgba(239,68,68,0.12),transparent 30%),radial-gradient(circle at bottom left,rgba(249,115,22,0.08),transparent 30%),linear-gradient(180deg,#020617 0%,#0f172a 100%);color:#f8fafc;font-family:Arial,sans-serif;">
        <div style="width:100%;max-width:840px;background:rgba(15,23,42,0.96);border:1px solid rgba(239,68,68,0.22);border-radius:28px;padding:32px;box-shadow:0 24px 70px rgba(0,0,0,0.45);position:relative;overflow:hidden;">
          ...
          <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.16);border-radius:20px;padding:18px;margin-bottom:18px;">
            ...
            <div style="font-size:14px;line-height:1.8;color:#e2e8f0;">This source has been blocked and disabled. A safe browser tab has been opened automatically.</div>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:18px;">
            <div style="background:rgba(2,6,23,0.55);border:1px solid #334155;border-radius:18px;padding:16px;">
              ...
              <ul style="margin:0;padding-left:18px;color:#cbd5e1;line-height:1.8;font-size:14px;"><li>High-risk interaction blocked</li><li>Threat source blacklisted</li><li>Safe tab opened automatically</li><li>User protected before exploitation</li></ul>
            </div>
            <div style="background:rgba(2,6,23,0.55);border:1px solid #334155;border-radius:18px;padding:16px;">
          ...
          <div style="background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);border-radius:18px;padding:16px;color:#bbf7d0;font-weight:800;">✅ Protected by Chrome Cyber Safe • Safe recovery completed</div>
        </div>
      </div>
    `;
  }
  ...
  function showSisterModal(blockedUrl, mode, riskScore) {
    ...
    const redirectText = mode === "phishing" ? "Opening a safe browser tab and quarantining this threat page..." : "Returning safely to the previous page...";

    const card = document.createElement("div");
    card.style.cssText = "width:100%;max-width:760px;background:rgba(15,23,42,0.98);color:#f8fafc;border:1px solid rgba(239,68,68,0.25);border-radius:28px;padding:28px;box-shadow:0 25px 80px rgba(0,0,0,0.55);font-family:Arial,sans-serif;position:relative;overflow:hidden;animation:ssFadeIn 0.35s ease;";
    ...
    card.innerHTML = `
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:8px;margin-bottom:18px;">
        <div style="display:flex;align-items:center;gap:14px;">
          ...
          <div><div style="font-size:26px;font-weight:800;">Chrome Cyber Safe Blocked This</div><div style="font-size:14px;color:#cbd5e1;">${threatLabel}</div></div>
        </div>
        <div style="padding:10px 16px;border-radius:999px;background:rgba(239,68,68,0.14);border:1px solid rgba(239,68,68,0.22);font-size:13px;font-weight:800;color:#fca5a5;">Threat Score: ${riskScore} / 100</div>
      </div>
      <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.16);border-radius:20px;padding:18px;margin-bottom:18px;">
    ...
    const timer = setInterval(() => {
      ...
      if (seconds <= 0) {
        clearInterval(timer);
        if (mode === "phishing") { window.open("https://www.google.com", "_blank"); overlay.remove(); showQuarantineScreen(); return; }
        overlay.remove();
      }
    }, 1000);
  }
  ...
  function flagMorphedImages() {
    ...
    document.querySelectorAll("[data-morph-risk='true']").forEach((img, index) => {
      ...
      badge.style.cssText = "position:absolute;top:10px;left:10px;z-index:1000000;background:linear-gradient(90deg,#7f1d1d,#991b1b);color:white;padding:8px 12px;border-radius:999px;font-size:11px;font-weight:800;box-shadow:0 8px 18px rgba(0,0,0,0.25);";
  ...
  function handleSocialAction(actionType, blockedUrl, riskScore, threatType) {
    isSocialPageBlacklisted((alreadyBlacklisted) => {
      ...
      if (actionType === "recover") { alert("⚠ Fake website detected by Chrome Cyber Safe. Access blocked."); return; }
      ...
      alert("⚠ Suspicious social action blocked by Chrome Cyber Safe.");
    });
  }
  ...
  function init() {
    ...
    setTimeout(() => {
      ...
      chrome.storage.local.set({ protectionStatus: "ACTIVE", lastRiskScore: CURRENT_RISK_SCORE });
      loader.remove();
