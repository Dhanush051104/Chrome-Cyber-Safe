(function () {
  ...
  // This will hold the backend score once it arrives
  ...
  // =============================================
  // FEATURE EXTRACTION — extension is data collector only
  // =============================================
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (response?.success) {
          ...
          // Backend is the ONLY place that sets the score
          ...
          // Now render UI with the real score
          updateUIWithBackendScore(backendScore, threat);

        } else {
          console.error("❌ Backend error:", response?.error);
          // If backend fails, show a neutral banner so page isn't blank
  ...
  function updateUIWithBackendScore(score, threat) {
    ...
    // Remove the loader if still showing
    const loader = document.getElementById("ss-loader");
    if (loader) loader.remove();

    // Remove any existing banner before drawing a new one
    const existingBanner = document.getElementById("ss-banner");
    if (existingBanner) existingBanner.remove();

    // Draw the banner with the REAL backend score
    createTopBanner(score, threat);

    // Wire up risk elements now that we have the real score
    setupRiskElements();

    // Social demo image flagging
  ...
  function createTopLoader() {
    // Remove any pre-existing loader first
    ...
    loader.innerHTML = `
      <div style="width:16px;height:16px;border:2px solid rgba(255,255,255,0.35);border-top-color:#fff;border-radius:50%;animation:ssSpin 0.8s linear infinite;"></div>
  ...
  function createTopBanner(riskScore, threat) {
    // Colour depends on threat level
    const isHigh = riskScore >= 75;
    const bgColor = isHigh
      ? "linear-gradient(90deg,#7f1d1d,#991b1b)"   // red for high risk
      : riskScore >= 40
        ? "linear-gradient(90deg,#78350f,#92400e)"  // amber for suspicious
        : "linear-gradient(90deg,#14532d,#166534)"; // green for safe

    const banner = document.createElement("div");
    banner.id = "ss-banner";
    banner.style.cssText = `position:fixed;top:0;left:0;right:0;z-index:999998;background:${bgColor};color:white;padding:12px 18px;font-family:Arial,sans-serif;font-size:14px;font-weight:700;box-shadow:0 2px 12px rgba(0,0,0,0.25);display:flex;justify-content:space-between;align-items:center;gap:10px;animation:ssFadeIn 0.35s ease;`;
    banner.innerHTML = `
  ...
  function setupRiskElements() {
    ...
    elements.forEach((el, index) => {
      ...
      el.addEventListener("click", function (e) {
        ...
        // Use CURRENT_RISK_SCORE — always the backend score by the time a user clicks
  ...
  // =============================================
  // INIT — clean linear flow, no local scoring
  // =============================================

  function init() {
    createStyles();

    // 1. Show loader while waiting for backend
    createTopLoader();

    // 2. Extract features
    const features = extractFeatureVector();

    // 3. Send to backend → backend responds → UI renders
    //    ALL UI rendering happens inside sendToBackend's callback
